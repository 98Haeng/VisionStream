...
stats_data = []
csv_columns = ['Frame', 'Total Count', 'Average Concentration'] + [
    'seeteacher', 'seemonitor', 'seebook', 'sleep', 'seeother', 'somethingelse', 'drinkwater']
...
def save_stats():
    ...
    global stats_data
    if stats_data:
        df = pd.DataFrame(stats_data, columns=csv_columns)
        for col in csv_columns:
            if col not in df.columns:
                df[col] = 0
        df = df[csv_columns]
        df.to_csv(stats_file_path, index=False, mode='a', header=not os.path.exists(stats_file_path))
        stats_data = []
...
while True:
    ...
    if frame_count % capture_frame_interval == 0:
        ...
        frame_stats = {
            'Frame': frame_count,
            'Total Count': total_count_per_frame,
            'Average Concentration': average_concentration,
        }
        for label in concentration_scores.keys():
            frame_stats[label] = label_counts[label] if label in label_counts else 0
        ...
        somethingelse_or_seeother_detected = sum(label_counts['somethingelse'], label_counts['seeother'])  >= 2
